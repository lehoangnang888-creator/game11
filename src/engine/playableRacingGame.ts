...
export class PlayableRacingGame {
  ...
  private buildTrack() {
    // Generate organic high-speed racing circuit
  ...
  private buildAIRivals() {
    const aiConfigs = [
      { id: 'ai_1', name: 'Apex Predator', color: 0xd97706, offset: 0.3, progress: 0.045 },
      { id: 'ai_2', name: 'Phantom Turbo', color: 0x9333ea, offset: -0.4, progress: 0.035 },
      { id: 'ai_3', name: 'Viper GT', color: 0x16a34a, offset: 0.2, progress: 0.025 },
      { id: 'ai_4', name: 'Crimson Fury', color: 0xe11d48, offset: -0.3, progress: 0.015 }
    ];
  ...
  public setupWeather(weather: WeatherType) {
    ...
    if (weather === 'SUNNY') {
      ...
      this.scene.fog = new THREE.FogExp2(0xfb923c, 0.0022);
      ...
      this.playerHeadlights.forEach(h => (h.intensity = 15));
      ...
      this.scene.fog = new THREE.FogExp2(0x475569, 0.004);
  ...
  private updatePlayerPhysics(delta: number) {
    ...
    // Drivetrain Gearbox Simulation
    const gearRatios = [3.82, 2.36, 1.69, 1.31, 1.0, 0.82];
    const maxSpeedsPerGear = [65, 110, 160, 210, 260, 315];
    ...
    // Acceleration & Drag
    const maxEnginePower = 480 * (1 - this.damagePct * 0.005); // Reduced by damage
    if (effectiveThrottle > 0) {
      const accelRate = (maxEnginePower / 1400) * 8.5 * (1.1 - this.playerSpeed / 320);
    ...
    const maxSteerSpeedFactor = Math.max(0.25, 1.0 - (this.playerSpeed / 340) * 0.75);
